  ...
  <div className="bg-brown-200 rounded-3xl p-6 lg:p-8 flex flex-col gap-4 border border-brown-300">
    <div className="flex items-center gap-4">
      ...
      <div className="flex flex-col">
        <span className="body-3 text-brown-400 uppercase tracking-widest text-[10px] lg:text-xs font-bold">Author</span>
...
function ViewPostPage() {
  ...
  const handleSendComment = () => {
    ...
    if (!comment.trim()) {
      return toast.custom((t) => (
        <div className="bg-red text-white p-5 rounded-2xl shadow-lg flex justify-between items-start w-full max-w-[350px] relative animate-in slide-in-from-right-5">
          <div className="flex flex-col gap-1 font-poppins">
            <h3 className="text-xl font-bold">Error!</h3>
            <p className="text-white/90 text-sm">Please write something before sending.</p>
          </div>
          <button onClick={() => toast.dismiss(t)} className="text-white/80 hover:text-white cursor-pointer transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
      ), { duration: 3000 });
    ...
    // 3. จำลองการส่งสำเร็จ
    toast.custom((t) => (
      <div className="bg-green text-white p-5 rounded-2xl shadow-lg flex justify-between items-start w-full max-w-[350px] relative animate-in slide-in-from-right-5">
        <div className="flex flex-col gap-1 font-poppins">
          <h3 className="text-xl font-bold">Success!</h3>
          <p className="text-white/90 text-sm">Your comment has been sent successfully.</p>
        </div>
        <button onClick={() => toast.dismiss(t)} className="text-white/80 hover:text-white cursor-pointer transition-colors">
          <X className="w-5 h-5" />
        </button>
      ...
      toast.custom((t) => (
        <div className="bg-green text-white p-5 rounded-2xl shadow-lg flex justify-between items-start w-full max-w-[350px] relative">
          <div className="flex flex-col gap-1 font-poppins">
            ...
            <p className="text-white/90 text-sm">This article has been copied to your clipboard.</p>
          </div>
          <button onClick={() => toast.dismiss(t)} className="text-white/80 hover:text-white cursor-pointer"><X className="w-5 h-5" /></button>
        </div>
      ), { duration: 3000 });
    });
  };
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-40 gap-4 font-poppins">
      ...
      <p className="text-brown-400 animate-pulse font-medium body-1">Loading article...</p>
    </div>
  );

  if (!post) return (
    <div className="text-center py-40 font-bold text-xl text-brown-600 font-poppins">Post not found.</div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 lg:p-10 flex flex-col gap-6 lg:gap-10 font-poppins">
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent className="bg-brown-100 rounded-[32px] border-none p-10 w-[90%] max-w-[621px] flex flex-col items-center">
          ...
          <AlertDialogHeader className="flex flex-col items-center gap-4">
            ...
            <AlertDialogDescription className="sr-only">Please create an account or login to proceed.</AlertDialogDescription>
          </AlertDialogHeader>
          <div className="flex flex-col items-center gap-6 mt-8 w-full">
            ...
            <div className="flex items-center gap-2">
              <span className="text-brown-400 body-1">Already have an account?</span>
              <Button variant="link" onClick={() => navigate("/login")} className="text-brown-600 body-1 underline underline-offset-4 cursor-pointer p-0 h-auto font-bold">
                Log in
              </Button>
            </div>
          </div>
      ...
      {/* 1. Featured Image */}
      <img src={post.image} className="w-full h-56 sm:h-72 lg:h-[500px] object-cover rounded-[32px] shadow-sm" alt={post.title} />

      {/* 2. Content Wrapper */}
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-start">
        <div className="flex-1 flex flex-col gap-6 w-full">
          <div className="flex items-center gap-2">
            <span className="bg-green-100 rounded-full px-3 py-1 body-2 text-green font-semibold">{post.category}</span>
            <span className="text-brown-200">|</span>
            <span className="body-1 text-brown-400">{formatDate(post.date)}</span>
          </div>

          <h1 className="text-3xl lg:text-5xl font-bold text-brown-600 leading-tight">{post.title}</h1>
          <p className="text-lg lg:text-xl text-brown-500 leading-relaxed italic border-l-4 border-brown-300 pl-4">{post.description}</p>
          
          <div className="markdown prose prose-stone max-w-none text-brown-600 leading-relaxed body-1">
            <ReactMarkdown>{post.content}</ReactMarkdown>
          </div>

          <div className="lg:hidden mt-4"><AuthorCard post={post} /></div>
          ...
          <div className="bg-brown-100 flex flex-col w-full rounded-[32px] p-6 md:p-10 gap-6 border border-brown-200 mt-8 shadow-sm">
            <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 md:gap-6">
              ...
              >
                <Smile className={`w-6 h-6 ${isLoggedIn ? 'text-brown-500' : 'text-brown-400'}`} /> {post.likes || 0}
              </Button>

              <div className="flex flex-col md:flex-row items-center justify-end md:flex-1 gap-4">
                <Button variant="outline" className="w-full md:w-auto h-12 px-8 rounded-full bg-white border-brown-300 text-brown-600 font-bold flex items-center justify-center gap-2 hover:bg-brown-50 cursor-pointer shadow-sm" onClick={handleCopyLink}>
                  <Copy className="w-4 h-4 text-brown-400" /> Copy link
                </Button>
                <div className="flex items-center gap-3">
                  <a href="#" className="bg-[#1877F2] p-3 rounded-full text-white hover:scale-110 transition-transform cursor-pointer shadow-md"><Facebook className="w-5 h-5 fill-current" /></a>
                  <a href="#" className="bg-[#0077B5] p-3 rounded-full text-white hover:scale-110 transition-transform cursor-pointer shadow-md"><Linkedin className="w-5 h-5 fill-current" /></a>
                  <a href="#" className="bg-[#1DA1F2] p-3 rounded-full text-white hover:scale-110 transition-transform cursor-pointer shadow-md"><Twitter className="w-5 h-5 fill-current" /></a>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid w-full gap-4 mt-12">
            <Label htmlFor="message" className="text-xl font-bold text-brown-600">Comment</Label>
            <Textarea
          ...
          <div className="flex flex-col gap-10 mt-10 mb-20">
            {mockComments.map((item, index) => (
              <div key={item.id} className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="flex items-center gap-4">
                  <img src={item.avatar} alt={item.user} className="w-12 h-12 rounded-full object-cover border border-brown-100 shadow-sm" />
                  <div className="flex flex-col">
                    <span className="font-bold text-brown-600 text-lg leading-tight">{item.user}</span>
                    <span className="text-sm text-brown-300">{item.date}</span>
                  </div>
                </div>
                <p className="text-brown-500 body-1 leading-relaxed">{item.content}</p>
                {index !== mockComments.length - 1 && <hr className="border-brown-100 mt-4" />}
              </div>
            ))}
          </div>
        </div>
